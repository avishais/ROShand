# From https://github.com/kwea123/hindsight_experience_replay
# Look also at: https://github.com/buntyke/her
...
class DDPGAgent:
    def __init__(self, state_size, action_size, goal_size, action_low=-1, action_high=1, gamma=0.98,
                 actor_learning_rate=0.01, critic_learning_rate=0.01, tau=1e-3, load_saved_net=False):
        ...
        self.load_saved_net = load_saved_net
    ...
    def _construct_nets(self):
        ...
        self.saved_net = '/home/pracsys/catkin_ws/src/rutgers_collab/src/rl_pkg/src/ddpg_her/log/net.ckpt'
        self.saver = tf.train.Saver()

        # $ tensorboard --logdir=logs
        # http://0.0.0.0:6006/
        self.writer = tf.summary.FileWriter('/home/pracsys/catkin_ws/src/rutgers_collab/src/rl_pkg/src/ddpg_her/log/', self.sess.graph)
        
        self.sess.run(tf.global_variables_initializer())

        if self.load_saved_net and os.path.isfile(self.saved_net + '.meta'):
            try:
    ...
    def log2summary(self, total_episodes, success_rate, a_losses, ep_mean_r):
        
        self.writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag='Actor losses', simple_value=a_losses)]), total_episodes)
        self.writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag='Success_rate', simple_value=success_rate)]), total_episodes)
        self.writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag='Mean reward', simple_value=ep_mean_r)]), total_episodes)
