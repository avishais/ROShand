...
bool Gripper::callbackVelRef(common_msgs_gl::SendDoubleArray::Request& req, common_msgs_gl::SendDoubleArray::Response& res){
	...
	if(velocity_mode_on_ == false){
		if(!initial_motor_pos_set_)
			motor_pos_ref_ = cur_gripper_pos_;//readData();
		else
...
void Gripper::callbackRefresehGripperPose(std_msgs::Float32MultiArray msg) {
...
void Gripper::velocityModeStep(){
	double rate = 1000;//100; // With the decrease, the velocity increases.
	auto motor_pos = cur_gripper_pos_;//readData();
	// std::vector<float> motor_pos = {0., 0.};
...
void Gripper::publishCommand(std::vector<double> commands){
...
void Gripper::publishRefZero() {
	std::vector<float> cur_pos = cur_gripper_pos_;
	std::vector<float> zero_vel_ref = {0.,0.};
	std_msgs::Float64MultiArray msg_pos, msg_vel;
	msg_pos.data.assign(cur_pos.begin(), cur_pos.end());
	pub_pos_ref_.publish(msg_pos);
	msg_vel.data.assign(zero_vel_ref.begin(), zero_vel_ref.end());
	pub_vel_ref_.publish(msg_vel);
}
