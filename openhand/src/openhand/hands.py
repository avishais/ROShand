...
class OpenHand():
	...
	amnt_release = 0	#values [0,1.0] for degree of closing and opening
	amnt_close = 0.5
...
class Model_M(OpenHand):
	...
	modes = [True] #True if in position control
...
class Model_T42(OpenHand):
	...
	def __init__(self,port="/dev/ttyUSB0",s1=1,s2=2,dyn_model="RX", motor_min_1= 0.05, motor_min_2=0.05):
		#s1: "forefinger"
		#s2: "thumb"
		OpenHand.__init__(self,port,[s1,s2],dyn_model)
	...
	def release(self):
		self.moveMotor(0,self.amnt_release + 0.37) # (+) Added by Avishai - should match the finger_opening_position in gripper.yaml
		self.moveMotor(1,self.amnt_release + 0.2)

	#model-specific OpenHand commands:
	def flip_init(self):
		self.moveMotor(0,self.amnt_release + initial_offset[0])
